...
class DataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, system: System, M: np.ndarray, K: np.ndarray, a: int, b: int, N: int , samples: int, limits_normal: np.ndarray, seed: int, PINN_sample_mode: str = 'split traj', data_gen_mode: str = 'negative forward', pretrained_T = None) -> None:
        super().__init__()
        self.M = M
        self.K = K
        self.system = system
        ...
        if pretrained_T != None:
            pretrained_T = pretrained_T.to(self.device)
            self.train_data = self.generate_data_pretrained(pretrained_T)
        else:
            self.train_data = self.generate_data()    # Generate synthetic data x, z, y

        self.data_length = self.train_data[0].shape[0]*self.train_data[0].shape[1]         # Total number of samples
        x = torch.from_numpy(self.train_data[0]).reshape(self.data_length, system.x_size)#.requires_grad_()  # Convert to tensors and reshape
        z = torch.from_numpy(self.train_data[1]).reshape(self.data_length, system.z_size)#.requires_grad_()

        # Check if output is vector or scalar
        if self.train_data[2].ndim > 2:
            y = torch.from_numpy(self.train_data[2]).view(self.train_data[2].shape[0]*self.train_data[2].shape[1], self.train_data[2].shape[2])#.requires_grad_()     # y data
        else:
            y = torch.from_numpy(self.train_data[2]).view(self.train_data[2].shape[0]*self.train_data[2].shape[1])#.requires_grad_()
        ...
        if PINN_sample_mode == 'split set':
            ...
            # ----------------------- Physics loss data ----------------------- 
            self.train_data_ph = self.generate_data()
            self.data_length_ph = self.train_data_ph[0].shape[0]*self.train_data_ph[0].shape[1]         # Total number of samples
            self.x_data_ph = torch.from_numpy(self.train_data_ph[0]).view(self.data_length_ph, system.x_size)#.requires_grad_()
            self.z_data_ph = torch.from_numpy(self.train_data_ph[1]).view(self.data_length_ph, system.z_size)#.requires_grad_()
            self.ic_ph = self.train_data_ph[4]
            # Check if output is vector or scalar    
            if self.train_data_ph[2].ndim > 2:
                self.output_data_ph = torch.from_numpy(self.train_data_ph[2]).view(self.train_data_ph[2].shape[0]*self.train_data_ph[2].shape[1], self.train_data_ph[2].shape[2])#.requires_grad_()     # y data
            else:
                self.output_data_ph = torch.from_numpy(self.train_data_ph[2]).view(self.train_data_ph[2].shape[0]*self.train_data_ph[2].shape[1])#.requires_grad_()
            #--------------------------------------------------------------
    ...
    def generate_data_pretrained(self, pretrained_T):
        """
        Generates training data with z trajectories initialized with an available forward T map.
        """
        h = (self.b-self.a)/self.N
        ic = self.system.sample_ic(self.limits_normal, self.samples, seed = self.seed)     # Sample initial conditions for x
        ...
        with torch.no_grad():
            z0 = pretrained_T(ic)
        z0 = z0.cpu().numpy()
        z_data_fw2 = data.KKL_observer_data(self.M, self.K, output_fw, self.a, self.b, z0, self.N)
    ...
    def generate_data(self) -> tuple:
        ...
        ic = self.system.sample_ic(self.limits_normal, self.samples, seed = self.seed)     # Sample initial conditions for x
        x_data_fw, output_fw, t_fw = self.system.generate_data(ic, self.a, self.b, self.N)      # Forward simulation
        num_traj_samples = x_data_fw.shape[1]    # Number of samples in each trajectory

        if self.data_gen_mode == 'forward':
            cutoff_ind = int(0.1*num_traj_samples)
            ic_z = np.random.rand(self.samples, self.system.z_size)
            z_data_fw = data.KKL_observer_data(self.M, self.K, output_fw, self.a, self.b, ic_z, self.N)
            x_data_fw = x_data_fw[:,cutoff_ind:,:]     # Cutoff the beginning of the trajectory
            z_data_fw = z_data_fw[:,cutoff_ind:,:]     # Cutoff the beginning of the trajectory
