...
def main():
    ...
    trainset = DataSet(rossler, M, K, a, b, N, num_ic, limits, 8888, PINN_sample_mode = 'split traj', data_gen_mode = 'forward')
    ...
    T_net = NN(num_hidden, hidden_size, x_size, z_size, activation, normalizer=normalizer)    
    ...
    # Save the model
    save_model(T_net, config, out_dir / f'rossler_T_forward_lr{learning_rate}_batch{batch_size}_epoch{epochs}')
    # -----------------------------------------------------
    ...
    trainset = DataSet(rossler, M, K, a, b, N, num_ic, limits, 8888, PINN_sample_mode = 'no physics', data_gen_mode = 'negative forward', pretrained_T=T_net)
    print(f'Dataset created with {len(trainset.x_data)} datapoints.')
    # ------------------------------------------------------------------------ 
    ...
    activation = F.relu
    normalizer = Normalizer(trainset)
    T_inv_net = NN(num_hidden, hidden_size, z_size, x_size, activation, normalizer=normalizer)    
    ...
    # Save the model
    save_model(T_inv_net, config, out_dir / f'rossler_T_inverse_lr{learning_rate}_batch{batch_size}_epoch{epochs}')
   # ------------------------------------------------------
