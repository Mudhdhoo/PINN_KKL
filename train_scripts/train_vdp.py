...
def main():
    ...
    trainset = DataSet(vdp, M, K, a, b, N, num_ic, limits_normal, 8888, PINN_sample_mode = 'split traj', data_gen_mode = 'forward')
    ...
    # Save the model
    save_model(T_net, config, out_dir / f'vdp_T_forward_lr{learning_rate}_batch{batch_size}_epoch{epochs}')
    # -----------------------------------------------------

    # --------------------- Train Inverse --------------------

    print('Generating inverse dataset.')
    trainset = DataSet(vdp, M, K, a, b, N, num_ic, limits_normal, 8888, PINN_sample_mode = 'no physics', data_gen_mode = 'negative forward', pretrained_T=T_net)
    print(f'Dataset created with {len(trainset.x_data)} datapoints.')
    # ------------------------------------------------------------------------ 
    ...
    # Save the model
    save_model(T_inv_net, config, out_dir / f'vdp_T_inverse_lr{learning_rate}_batch{batch_size}_epoch{epochs}')
   # ------------------------------------------------------
