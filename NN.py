...
# Model NN1 and NN2
class NN(nn.Module):
    def __init__(self, num_hidden, hidden_size, in_size, out_size, activation, dropout_prob = 0.0, normalizer = None):
        super().__init__()
...
class Main_Network(nn.Module):
    def __init__(self, x_size, z_size, num_hidden, hidden_size, activation, mask = None, normalizer = None):
        super().__init__()
        self.normalizer = normalizer
        self.mode = 'normal'
    ...
    def forward(self, x):
        ...
        if self.normalizer != None:
           ...
           # norm_z_hat = self.normalizer.Normalize(z_hat, self.mode).float()
        else:
