...
def KKL_observer_data(M, K, y, a, b, ic, N):
    ...
    if y.ndim > 2:             
        f = lambda y,z: np.matmul(M,z) + np.matmul(K, np.expand_dims(y,1))  # Reshape y from (m,) --> (m, 1) for matrix multiplication
    else:
...
def calc_p(e, nx, bounds):
    # Calculate volume of state space
    vol = 1
    for bound in bounds:
        vol = vol*(bound[1] - bound[0])

    p = vol*math.gamma(nx/2 + 1) / (math.pi**(nx/2)*e**nx)
