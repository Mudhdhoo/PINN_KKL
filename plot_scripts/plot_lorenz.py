...
#------------- Load model for Lorenz Attractor and initialize observer ------------- 
rho = 28
sigma = 10
beta = 8/3
...
lorenz = Lorenz(rho, sigma, beta, noise_measurement_std=2, noise_process_std=2)

model_data = torch.load('models/lorenz_T_inverse_lr0.001_batch32_epoch15', map_location=device)
...
observer = Observer(lorenz, M, K, T_inv_net, a, b, N, init_z_zero=True)
...
# -------------------------- Save Data -------------------------- 
df = pd.DataFrame(x[:,0])
df.to_excel("data/lorenz/lorenz_x1.xlsx", index=False, header=False)

df = pd.DataFrame(x[:,1])
df.to_excel("data/lorenz/lorenz_x2.xlsx", index=False, header=False)

df = pd.DataFrame(x[:,2])
df.to_excel("data/lorenz/lorenz_x3.xlsx", index=False, header=False)

df = pd.DataFrame(x_hat[:,0])
df.to_excel("data/lorenz/lorenz_x1_hat.xlsx", index=False, header=False)

df = pd.DataFrame(x_hat[:,1])
df.to_excel("data/lorenz/lorenz_x2_hat.xlsx", index=False, header=False)

df = pd.DataFrame(x_hat[:,2])
df.to_excel("data/lorenz/lorenz_x3_hat.xlsx", index=False, header=False)
...
plt.yticks(np.arange(-20, 20+10, 10))
...
plt.yticks(np.arange(-10, 50+20, 20))
plt.xlabel('Time')
plt.legend(prop = {'size':legend_fontsize}, loc = 'best')

plt.savefig(f'figures/lorenz/lorenz.png', dpi=700, transparent = False, bbox_inches = 'tight', facecolor = 'w')
